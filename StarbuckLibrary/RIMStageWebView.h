...
#ifndef RIMSTAGEWEBVIEW_H
#define RIMSTAGEWEBVIEW_H
...
//	virtual void injectJavaScriptObj(const QObject* jsObj) = 0;
};
...
class RIMStageWebView :	public WebView, public IRIMStageWebView
{
	Q_OBJECT

public:
	RIMStageWebView(QSize viewsize, QWidget *parent = 0);
	~RIMStageWebView(void);

	void registerLocationChangeLisenter(QObject *obj, char *method);
  ...
  void injectJavaScriptObj(const QObject* jsObj);
	...
	/**
     * The area on the stage in which the <code>RIMStageWebView</code> object is displayed.     
     */
	QRect viewPort();

	/**
     * Set the area on the stage in which the <code>RIMStageWebView</code> object is displayed.   
	...
	/**
     * The private browsing flag tells the <code>RIMStageWebView</code> object to enable private browsing mode.
...
signals:
	void documentCreatedForFrame();

protected:
	//void mouseReleaseEvent ( QMouseEvent * ev );
	void notifyDocumentCreatedForFrame(const WebFrame frame, const bool isMainFrame, const WebDOMDocument& document, const JSContextRef context, const JSValueRef window);
